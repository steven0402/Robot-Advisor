const INDICATOR_CACHE_TTL_MS = Number(
  process.env.INDICATOR_CACHE_TTL_MS || 30 * 60 * 1000
);
...
function buildSummaryCacheKey(indicatorData, modelName, hasApiKey) {
...
async function getCachedSummary(loader, indicatorData, modelName, hasApiKey) {
  const cacheKey = buildSummaryCacheKey(indicatorData, modelName, hasApiKey);
