...
function buildSeries(stockSeries, btcPriceMap, matchKey) {
  return stockSeries
    .map((stockPoint) => {
      ...
      return {
        date: stockPoint.date || stockPoint.displayDate?.slice(0, 10) || null,
...
function mergeIntradayIntoDailySeries(dailySeries, intradaySeries) {
  if (!dailySeries.length) {
    return intradaySeries;
  }

  if (!intradaySeries.length) {
    return dailySeries;
  }
  ...
  if (latestIntraday.date === latestDaily.date) {
    return [...dailySeries.slice(0, -1), latestIntraday];
  }

  return [...dailySeries, latestIntraday];
}
...
async function getIndicatorData() {
  ...
  if (btcDailyResult.status !== "fulfilled" || stockDailyResult.status !== "fulfilled") {
    const errors = [
      btcDailyResult.status === "rejected" ? `BTC daily: ${btcDailyResult.reason.message}` : null,
      stockDailyResult.status === "rejected"
        ? `MSTR daily: ${stockDailyResult.reason.message}`
        : null
    ].filter(Boolean);
  ...
  const btcDailySeries = btcDailyResult.value;
  const stockDailySeries = stockDailyResult.value;
  const btcIntradaySeries =
    btcIntradayResult.status === "fulfilled" ? btcIntradayResult.value : [];
  const stockIntradaySeries =
    stockIntradayResult.status === "fulfilled" ? stockIntradayResult.value : [];
  ...
  const displaySeries = mergeIntradayIntoDailySeries(dailySeries, intradaySeries);

  const baseSummary = summarizeSeries(displaySeries);
  ...
  return {
    metadata: {
      ...
      sourceStatus: {
        btcDaily: btcDailyResult.status,
        stockDaily: stockDailyResult.status,
        btcIntraday: btcIntradayResult.status,
        stockIntraday: stockIntradayResult.status
      },
      intradayInterval: "5m",
